...
public class ConfigControllerUnitTest extends AbstractControllerUnitTest {
    ...
    public void setUp() throws Exception {
        ...
        this.configService.setTaskManagerService(taskManagerService);
        
        NotifyTaskProcessor notifyProcessor = new NotifyTaskProcessor();
        notifyProcessor.setNotifyService(notifyService);
        
        this.configService.setNotifyTaskProcessor(notifyProcessor);
    }
    ...
    public void testGetConfig_默认分组() {
        ...
        ConfigInfo configInfo = new ConfigInfo("dataId1", Constants.DEFAULT_GROUP, "test");
        this.configService.updateMD5Cache(configInfo);
        assertEquals("forward:/" + Constants.BASE_DIR + "/" + Constants.DEFAULT_GROUP + "/dataId1", configController
            .getConfig(request, response, "dataId1", null));
    }


    @Test
    public void testGetConfig_服务器设定分组() {
        this.groupService.addAddress2GroupMapping(MockHttpServletRequest.DEFAULT_REMOTE_ADDR, "dataId1", "notify");
        MockHttpServletRequest request = new MockHttpServletRequest();
        MockHttpServletResponse response = new MockHttpServletResponse();
        ConfigInfo configInfo = new ConfigInfo("dataId1", "notify", "test");
        this.configService.updateMD5Cache(configInfo);
        assertEquals("forward:/config-data/notify/dataId1", configController.getConfig(request, response, "dataId1",
            null));
    }
    ...
    public void testGetConfig_客户端定义分组() {
        MockHttpServletRequest request = new MockHttpServletRequest();
        MockHttpServletResponse response = new MockHttpServletResponse();
        ConfigInfo configInfo = new ConfigInfo("dataId1", "test", "test content");
        this.configService.updateMD5Cache(configInfo);
        assertEquals("forward:/config-data/test/dataId1", configController.getConfig(request, response, "dataId1",
            "test"));
    }
    ...
    public void testGetConfig_400() {
        ...
        MockHttpServletResponse response = new MockHttpServletResponse();
        assertEquals("400", configController.getConfig(request, response, "dataId1", "test"));
    ...
    public void testGetConfig_503() {
        ...
        assertEquals("503", configController.getConfig(request, response, "dataId1", "test"));
    ...
    public void testGetConfig_404() {
        MockHttpServletRequest request = new MockHttpServletRequest();
        MockHttpServletResponse response = new MockHttpServletResponse();
        assertEquals("404", configController.getConfig(request, response, "dataId1", "test404"));
    }
    ...
    public void testGetConfig_304() {
        ...
        this.configService.updateMD5Cache(configInfo);
        this.diskService.getModifyMarkCache().put(this.diskService.generateCacheKey("test1", "dataId1"), true);
        assertEquals("304", configController.getConfig(request, response, "dataId1", "test1"));
    ...
    public void testGetConfig_md5() {
        ...
        assertEquals("forward:/config-data/test/dataId1", configController.getConfig(request, response, "dataId1",
            "test"));
        assertEquals(MD5.getInstance().getMD5String("hello world"), response.getHeader(Constants.CONTENT_MD5));
    ...
    public void testGetConfigKeyList() {
        ...
        configKeyList =
                this.configController.getConfigKeyList("dataId1" + WORD_SEPARATOR + "group1" + WORD_SEPARATOR + "md51"
                        + LINE_SEPARATOR + "dataId2" + WORD_SEPARATOR + "group2" + WORD_SEPARATOR + "md52"
                        + LINE_SEPARATOR + "dataId2" + WORD_SEPARATOR + "group2" + WORD_SEPARATOR + "md52"
                        + LINE_SEPARATOR);
        Assert.assertEquals(3, configKeyList.size());

        configKeyList =
                this.configController.getConfigKeyList("dataId1" + WORD_SEPARATOR + WORD_SEPARATOR + "md51"
                        + LINE_SEPARATOR + "dataId2" + WORD_SEPARATOR + "group2" + WORD_SEPARATOR + "md52"
                        + LINE_SEPARATOR + "dataId2" + WORD_SEPARATOR + "group2" + WORD_SEPARATOR + "md52"
                        + LINE_SEPARATOR);
        Assert.assertEquals(3, configKeyList.size());
        ...
        configKeyList =
                this.configController.getConfigKeyList("dataId1" + WORD_SEPARATOR + "group1" + WORD_SEPARATOR
                        + LINE_SEPARATOR + "dataId2" + WORD_SEPARATOR + "group2" + WORD_SEPARATOR + "md52"
                        + LINE_SEPARATOR + "dataId2" + WORD_SEPARATOR + "group2" + WORD_SEPARATOR + "md52"
                        + LINE_SEPARATOR);
        Assert.assertEquals(3, configKeyList.size());
        ...
        configKeyList =
                this.configController.getConfigKeyList("dataId1:;;;" + WORD_SEPARATOR + WORD_SEPARATOR + LINE_SEPARATOR
                        + "dataId2" + WORD_SEPARATOR + "group2" + WORD_SEPARATOR + "md52" + LINE_SEPARATOR + "dataId2"
                        + WORD_SEPARATOR + "group2" + WORD_SEPARATOR + "md52" + LINE_SEPARATOR);
        Assert.assertEquals(3, configKeyList.size());
        
        StringBuilder buf = new StringBuilder();
        buf.append("aaa:;:").append(WORD_SEPARATOR).append("groupId1:").append(WORD_SEPARATOR).append("md1").append(LINE_SEPARATOR);
        buf.append("bbb:;:").append(WORD_SEPARATOR).append("groupId2:").append(WORD_SEPARATOR).append("md2").append(LINE_SEPARATOR);
        buf.append("ccc:;:").append(WORD_SEPARATOR).append(WORD_SEPARATOR).append("md3").append(LINE_SEPARATOR);
    ...
    public void testGetRemoteIp() {
        ...
        Assert.assertEquals("192.168.0.1", this.configController.getRemortIP(request));
        
        request = new MockHttpServletRequest();
        Assert.assertEquals("127.0.0.1", this.configController.getRemortIP(request));
    }
    ...
    public void testGetProbe_400() {
        ...
        MockHttpServletResponse response = new MockHttpServletResponse();
        request.setRemoteAddr(null);
        assertEquals("400", configController.getProbeModifyResult(request, response, "dataId1" + WORD_SEPARATOR + "test"));
        assertEquals(HttpServletResponse.SC_BAD_REQUEST, response.getStatus());
    }
    ...
    public void testGetProbe_503() {
        ...
        MockHttpServletResponse response = new MockHttpServletResponse();
        GlobalCounter.getCounter().set(1);
        assertEquals("503", configController.getProbeModifyResult(request, response, "dataId1" + WORD_SEPARATOR + "test"));
        assertEquals(HttpServletResponse.SC_SERVICE_UNAVAILABLE, response.getStatus());
        assertEquals(0, GlobalCounter.getCounter().get());
    }
    ...
    public void testGetProbe_MD5不为空且和缓存中的MD5值不同() throws Exception {
        ...
        String probeString = dataId + WORD_SEPARATOR + group + WORD_SEPARATOR + probeMd5 + LINE_SEPARATOR;
        String expected = dataId + WORD_SEPARATOR + group + LINE_SEPARATOR;
        expected = URLEncoder.encode(expected, "UTF-8");
        
        Assert.assertEquals("200", this.configController.getProbeModifyResult(request, response, probeString));
    ...
    public void testGetProbe_MD5不为空且和缓存中的MD5相同() {
        ...
        Assert.assertEquals("", response.getHeader(Constants.PROBE_MODIFY_RESPONSE_NEW));
    }
    ...
    public void testGetProbe_MD5为空且缓存中的MD5也为空() {
        ...
        String probeString = dataId + WORD_SEPARATOR + group + WORD_SEPARATOR + LINE_SEPARATOR;
    ...
    public void testGetProbe_老版本客户端() throws Exception {
        ...
        Assert.assertEquals("200", this.configController.getProbeModifyResult(request, response, probeString));
        Assert.assertEquals(expected, response.getHeader(Constants.PROBE_MODIFY_RESPONSE_NEW));
    }
