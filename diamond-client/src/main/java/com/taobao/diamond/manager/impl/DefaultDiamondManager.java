...
public class DefaultDiamondManager implements DiamondManager {
    ...
    private final String instanceId = String.valueOf(INSTANCE_ID.getAndIncrement());
    ...
    private DefaultDiamondManager(Builder builder) {
        ...
        diamondSubscriber = DiamondClientFactory.getSingletonDiamondSubscriber(Constants.DEFAULT_DIAMOND_CLUSTER);

        if (null != builder.diamondConfigure) {
            this.diamondSubscriber.setDiamondConfigure(builder.diamondConfigure);
        }
        ...
        ((DefaultSubscriberListener) diamondSubscriber.getSubscriberListener()).addManagerListeners(this.dataId,
            this.group, this.instanceId, this.managerListeners);
        diamondSubscriber.addDataId(this.dataId, this.group);
        diamondSubscriber.start();

        try {
            interestRealTimeNotification(dataId, group);
        }
        catch (Exception e) {
            log.error("注册实时通知dataId和group时出错", e);
        }
    }
     ...
     * @param clusterType
     */
    public DefaultDiamondManager(String group, String dataId, ManagerListener managerListener, String clusterType) {
        this.dataId = dataId;
        this.group = group;
    ...
    public DefaultDiamondManager(String group, String dataId, ManagerListener managerListener,
        ...
        this(group, dataId, managerListener, useRealTimeNotification, Constants.DEFAULT_DIAMOND_CLUSTER);
    }
    ...
    public DefaultDiamondManager(String group, String dataId, List<ManagerListener> managerListenerList) {
        this(group, dataId, managerListenerList, Constants.DEFAULT_DIAMOND_CLUSTER);
    }
    ...
    public DefaultDiamondManager(String group, String dataId, List<ManagerListener> managerListenerList,
            boolean useRealTimeNotification) {
        this(group, dataId, managerListenerList, useRealTimeNotification, Constants.DEFAULT_DIAMOND_CLUSTER);
    }
    ...
    public List<ManagerListener> getManagerListeners() {
        return ((DefaultSubscriberListener) this.diamondSubscriber.getSubscriberListener()).getManagerListenerList(
            dataId, group, instanceId);
    }
    ...
    private void interestRealTimeNotification(String dataId, String group) throws Exception {
        // 注册要接收实时通知的dataId和group
        this.diamondSubscriber.getPushitClient().interest(dataId, group);
    }
